...
const open = async (row) => {
  ...
  if (row.id) {
    ...
    const file = await getImageAsFile(imgUrl.value, formModel.value.cover_img)
  ...
  >
    ...
    <el-form :model="formModel" ref="formRef" label-width="100px">
      ...
      <el-form-item label="文章封面" prop="cover_img">
        <!-- 此处需要关闭 element-plus 的自动上传，不需要配置 action 等参数，只需要做前端的本地预览图片即可，无需在提交前上传图片
